...
│   ├── layouts/              # 页面骨架布局
│   ├── pages/                # 页面级模块
│   ├── store/                # Zustand store / view model / agent 抽象
│   ├── index.css             # 全局样式
│   └── index.tsx             # 应用入口
├── index.html
├── package.json
...
如果希望一开始就把结构定得更清楚，推荐直接使用下面这个细化版：
...
当前项目里这一层承担了核心业务编排，例如：

- `session store`：登录态、用户信息
- `message store`：消息流状态
- `Agent` / `LangGraphAgent`：聊天行为抽象与流式响应处理
- `message sender store`：输入区附属状态
...
- 样式类名保持语义化，不用 `box1`、`wrapper2` 这类无意义命名
...
- `Agent` 抽象：只有在会话式、流式、工具编排场景下保留
- `sls`、埋点、钱包、授权等业务模块：按真实业务接入

...
- Windows 下展示的是 Kadaclaw 生成的包装命令路径，不是 npm 全局目录。
...
- 如果 PowerShell、`npm`、PATH 或执行策略导致安装失败，优先建议 WSL2。
...
- 已支持安装，但仍应视为兼容路径。
- 重点检查 PowerShell、`npm`、PATH、执行策略，以及 `openclaw.cmd` 包装器是否可执行。
...
- Windows 安装后仍找不到命令：优先确认包装器是否生成在 `<app-local-data>/openclaw-runtime/bin/openclaw.cmd`，其次再查 PowerShell 和 npm 安装链路。
